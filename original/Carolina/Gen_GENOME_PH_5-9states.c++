...
// It colors Euchromatin Strong (TSS), weak (transcribed), heterochrom(HP1) and deserts (H1)
//
#include<iostream>
#include<math.h>
#include<stdlib.h>
...
int InitialConfg=0;
//1 == RANDOM WALK - RW; 2 == Asymmetric RW (stretched) - S; 3 == CRUMPLED inside SPHERE - C; 4 == Fractal globule - FG; 5 == mitotic - M
...
int main(int argc, char* argv[]){
  ...
  cout << "Input argv: [1] red (polym-PH) sites; [2] black (non-polym-PH) sites ; [3] 9-states model; [4] Type (RW,S,C,FG,M); [5] Replicas; [6] N.PHp [7] N.HP1p; [8] N.EUp; [9] Config (1=RW; 2=STR; 3=CRUMPL; 4=FG; 5=M)"<<endl;
  ntypepol=19;
  ...
  double Ly=L;
  double Lz=L;
  double Rnuc=pow((GenomeCG+nproteins)*1.0/(0.16*M_PI*4.0/3.0),1.0/3.0);
  ...
  for(int reps=1;reps<=RepsMax;reps++){
    ...
    for(int index_ch=0; index_ch<6; index_ch++){
      ...
      while(!readRed.eof()){
        ...
        //cout << Chromosome << " " <<  start << " " << end <<endl;
	  ...
	  for(int nnb=nstart; nnb<=nend; nnb++){
	    ...
	    //cout << "P AbuPC " << nnb << " " <<  AbundancePC[index_ch][nnb]<<endl;
            //cout << "P PC " << nnb << " " << PC[index_ch][nnb]<<endl;
	  }
        }
      //cin.get();
      }
	...
	if(Chromosome.compare(ChrNAME[index_ch])==0){
          ...
          for(int nnb=nstart; nnb<=nend; nnb++){ 
            ...
            if(nnb==nstart) AbundancePC[index_ch][nnb]= fabs(1.0-(start*1.0/coarsegrain-nstart));
            if(nnb==nend) AbundancePC[index_ch][nnb]= fabs(1.0-(nend-end*1.0/coarsegrain));
            //cout << "PB AbuPC " << nnb << " "<< AbundancePC[index_ch][nnb]<<endl;
            //cout << "PB PC " << nnb << " "<< PC[index_ch][nnb]<<endl;
          }
        }
        //cin.get();
      }
        ...
        if(Chromosome.compare(ChrNAME[index_ch])==0){
          if(typeChr==Euchr1a or typeChr==Euchr1b){
            ...
            for(int nnb=nstart; nnb<nend; nnb++){
              ...
              //cout << "AbuEU1 " << nnb <<" " << AbundanceEu[index_ch][nnb]<<endl;
            }
          }
          if(typeChr==Euchr2a or typeChr==Euchr2b){
            ...
            for(int nnb=nstart; nnb<nend; nnb++){
              ...
              if(nnb==nstart) AbundanceEu[index_ch][nnb]= fabs(1.0-(start*1.0/coarsegrain-nstart));
              if(nnb==nend-1) AbundanceEu[index_ch][nnb]= fabs(1.0-(nend-end*1.0/coarsegrain));
              //cout << "AbuEU2 " << nnb << " " << AbundanceEu[index_ch][nnb]<<endl;
            }
          }	
          if(typeChr==7 || typeChr==8){ //7-8 heterochrom
            ...
            for(int nnb=nstart; nnb<nend; nnb++){
              ...
              //cout << "AbuB " << nnb << " " << AbundanceHet[index_ch][nnb]<<endl;
            }
          }
          if(typeChr==9){ //9 low transcription
            ...
            for(int nnb=nstart; nnb<nend; nnb++){
              ...
              //cout << "AbuD " << nnb << " " << AbundanceDesert[index_ch][nnb]<<endl;
            }
          }
        }
      }
    ...
    for(int index_ch=0;index_ch<6;index_ch++){
      ...
      double xsh=0;
      double ysh=0;
      if(index_ch==0){xsh=-0.15;ysh=-0.25;}
      if(index_ch==1){xsh=-0.25;ysh=0.;}
      if(index_ch==2){xsh=+0.25;ysh=0.;}
      if(index_ch==3){xsh=+0.15;ysh=-0.25;}
      if(index_ch==4){xsh=0.0;ysh=0.0;}
      if(index_ch==5){xsh=0.0;ysh=+0.25;}
      ///////////////////////////////////////////////////////////
	  ...
	  if(m==0){
	    ...
	    while(distance(position[index_ch][m],center)>R1 or check==0){
	      ...
	      if(position[index_ch][m][0]>Lx*0.15 or position[index_ch][m][0]<-Lx*0.15) check=0;
	      if(position[index_ch][m][1]>Ly*0.125 or position[index_ch][m][1]<-Ly*0.125) check=0;
	      if(position[index_ch][m][2]>Lz/2. or position[index_ch][m][2]<-Lz/2.) check=0;
              if(index_ch==4 && distance(position[index_ch][m],center)>10) check=0;
	    }
	    theta[0]=theta[1];
	    phi[0]=phi[1];
        ...
        //cout << "distance " << distance(position[m],position[m-1])<<endl;
        for(int m=0;m<LChr[index_ch];m++){
          if(index_ch==4){writeW << atomcounter+1 << " " << 1 << " " << ColorChr[index_ch][m] << " " << position[index_ch][m][0]-com[0]+xsh*L <<" " << position[index_ch][m][1]-com[1]+ysh*L << " " << position[index_ch][m][2]-com[2]-Rnuc+12.0 << " " << 0 << " " << 0 << " " << 0 << endl;}
          else{writeW << atomcounter+1 << " " << 1 << " " << ColorChr[index_ch][m] << " " << position[index_ch][m][0]-com[0]+xsh*L <<" " << position[index_ch][m][1]-com[1]+ysh*L << " " << position[index_ch][m][2]-com[2] << " " << 0 << " " << 0 << " " << 0 << endl;}
          atomcounter++;
        }
      ...
      if(InitialConfg==5){ //Mitotic
        ...
        for(int m=0;m<LChr[index_ch];m++){
          ...
          if(index_ch==0 || index_ch==2){
            phi=(LChr[index_ch]-1-m)*1.0/300.0*2.0*M_PI;
          }else{
            phi=m*1.0/300.0*2.0*M_PI;
          }
        ...
        for(int m=0;m<LChr[index_ch];m++){
          if(index_ch==4){writeW << atomcounter+1 << " " << 1 << " " << ColorChr[index_ch][m] << " " << position[index_ch][m][0]-com[0]+xsh*L <<" " << position[index_ch][m][1]-com[1]+ysh*L << " " << position[index_ch][m][2]-com[2]-Rnuc+10.0 << " " << 0 << " " << 0 << " " << 0 << endl;}
          else{writeW << atomcounter+1 << " " << 1 << " " << ColorChr[index_ch][m] << " " << position[index_ch][m][0]-com[0]+xsh*L<<" " << position[index_ch][m][1]-com[1]+ysh*L << " " << position[index_ch][m][2]-com[2] << " " << 0 << " " << 0 << " " << 0 << endl;}
          atomcounter++;
        }
    ...
    //   MAke PROTEINS
    ///////////////////////////////////////////////////////////
    //cout << "atomcounter " << atomcounter<<endl;
    if(InitialConfg!=2){
      for(int m=0;m<PCprotein;m++){
        ...
        //cout << GenomeCG+m+1 <<endl;cin.get();
      }
    ...
    for(int t=0;t<tether;t++)writeW <<GenomeCG+nproteins+t+1<< " "<< 1 << " " << ntypepol+1 << " 0 0 " << -Rnuc+2.0 << " 0 0 0 "<<endl;
...
void Paint(int index_ch){
  for(int n=0;n<LChr[index_ch];n++){
    ...
    // 0 or Black or Eu1 or Eu2 AND PC: 0 or 1 or 2
    // 
    //Easy stuff: PC==0
    if(PC[index_ch][n]==0 && Het[index_ch][n]==0 && Eu[index_ch][n]==0 && Desert[index_ch][n]==0) ColorChr[index_ch][n]=1; //normal
    if(PC[index_ch][n]==0 && Het[index_ch][n]==1 && Eu[index_ch][n]==0 && Desert[index_ch][n]==0) ColorChr[index_ch][n]=4; //only-black
    ...
    if(PC[index_ch][n]==0 && Het[index_ch][n]==0 && Eu[index_ch][n]==0 && Desert[index_ch][n]==1) ColorChr[index_ch][n]=13; //only Desert
    // PC==1
    if(PC[index_ch][n]==1 && Het[index_ch][n]==0 && Eu[index_ch][n]==0 && Desert[index_ch][n]==0) ColorChr[index_ch][n]=2; //PC-high
    if(PC[index_ch][n]==1 && Het[index_ch][n]==1 && Eu[index_ch][n]==0 && Desert[index_ch][n]==0) ColorChr[index_ch][n]=5; //Black-PC1
    ...
    if(PC[index_ch][n]==2 && Het[index_ch][n]==1 && Eu[index_ch][n]==0 && Desert[index_ch][n]==0) ColorChr[index_ch][n]=6; //Black-PC2
