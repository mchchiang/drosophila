...
init_fraction=$(python -c "print $fraction/10.0")
...
sim_name="dros-equil_vfrac_${fraction}_run_${run}"
...
# Generate chromatin
${gen_chromo_exe} $chromo_file $hmm_file $ploidy $init_fraction $buffer "${run_dir}/${init_file}" "${run_dir}/${map_file}"
...
prep1_time_1=200 # 2000 run using soft + harmonic
prep1_time_2=400 # 4000 run using soft + fene
prep1_time_3=400 # 4000 run using lj/cut + fene

prep2_printfreq=100
prep2_seed=$(python GetRandom.py $max_seed)
prep2_time=5000 # 5000 run during which the box is reduced to desired volume

prep3_printfreq=100
prep3_seed=$(python GetRandom.py $max_seed)
prep3_time=5000 # equilibration after compression
...
commcutoff=$(python -c "print '%.13f' % ($L*1.5)")
...
sed -i -- "s/PREP1_TIME_0/${prep1_time_0}/g" $file
...
sed -i -- "s/COMMCUTOFF/${commcutoff}/g" $file
